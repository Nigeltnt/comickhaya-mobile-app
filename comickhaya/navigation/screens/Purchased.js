...
export default function Purchased() {
  ...
  const move = async (bookid, booknam) => {
    try {
      await AsyncStorage.setItem('bookdetails', bookid + '~' + booknam);
      ...
      <View style={styles.bookOuter}>
        ...
        <View style={{ flexDirection: 'column' }}>
          ...
          <View>
            <TouchableHighlight
              style={styles.bookPressable}
              onPress={() => move(`${item.bookid} `, `${item.book} `)}>
  ...
  //     let resp = await fetch(`${apiLink}books.php?id=${authId}`);
